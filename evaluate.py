import torch
import random
from train import sentence_to_index, bert_embedding, nn_embedding
...
def decode(decoder, decoder_hidden, encoder_outputs, voc, embedding_dict, max_length=MAX_LENGTH):
...
def add_new_word(sentence, voc):
    index = len(voc.loc_count)
    for num, value in enumerate(sentence.split(" ")):
        voc.count2word[index] = value
        voc.loc2index[(-1, num)] = index
        index += 1
        voc.loc_count += 1

def evaluate(encoder, decoder, voc, sentence, beam_size, hidden_size, max_length=MAX_LENGTH):
    embedding_dict = merge_embedding(sentence, voc, hidden_size)
    add_new_word(sentence, voc)
    row_sentence_pair = (-1, sentence)
    indexes_batch = [sentence_to_index(row_sentence_pair, voc)] #[1, seq_len]
    lengths = [len(indexes) for indexes in indexes_batch]
    input_batch = torch.LongTensor(indexes_batch).transpose(0, 1)
    input_batch = input_batch.to(device)

    encoder_outputs, encoder_hidden = encoder(input_batch, embedding_dict, lengths, None)

    decoder_hidden = encoder_hidden[:decoder.n_layers]

    if beam_size == 1:
        return decode(decoder, decoder_hidden, encoder_outputs, voc, embedding_dict)
    else:
...
def merge_embedding(sentence, voc, hidden_size):
    embedding_dict = {}
    # bert embedding
    bert_embedding(-1, sentence, embedding_dict, voc)
    # nn embedding
    nn_embedding(embedding_dict, hidden_size)
    return embedding_dict

def runTest(n_layers, hidden_size, batch_size, reverse, modelFile, beam_size, inp, corpus):
    ...
    attn_model = 'dot'
    decoder = LuongAttnDecoderRNN(attn_model, hidden_size, batch_size, voc.loc_count, n_layers)
