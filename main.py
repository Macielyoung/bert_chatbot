...
def parse():
    ...
    parser.add_argument('-p', '--printf', type=int, default=100, help='Print every p iterations')
...
def parseFilename(filename, test=False):
    ...
    batch_size = int(layers.split("-")[1])
    hidden_size = int(hidden)
    return n_layers, hidden_size, batch_size, reverse

def run(args):
    reverse, fil, n_iteration, print_every, save_every, learning_rate, \
        n_layers, hidden_size, batch_size, beam_size, inp, dropout = \
        args.reverse, args.filter, args.iteration, args.printf, args.save, args.learning_rate, \
    ...
    if args.train and not args.load:
        ...
        n_layers, hidden_size, batch_size, reverse = parseFilename(args.test, True)
        runTest(n_layers, hidden_size, batch_size, reverse, args.test, beam_size, inp, args.corpus)
