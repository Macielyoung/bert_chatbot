...
class EncoderRNN(nn.Module):
    def __init__(self, hidden_size, batch_size, n_layers=1, dropout=0):
    ...
    def forward(self, input_seq, embedding, input_lengths, hidden=None):
        embedded = self.obtain_embedding(input_seq, embedding)
    ...
    def obtain_embedding(self, input_seq, embedding):
        input_list = input_seq.view(-1).tolist()
        input_embedding_list = [embedding[loc] for loc in input_list]
        input_embed = torch.tensor(input_embedding_list).view(-1, self.batch_size, self.hidden_size)
        return input_embed
...
class LuongAttnDecoderRNN(nn.Module):
    def __init__(self, attn_model, hidden_size, batch_size, output_size, n_layers=1, dropout=0.1):
    ...
    def forward(self, input_seq, embedding, last_hidden, encoder_outputs):
